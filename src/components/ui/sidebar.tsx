/**
 * shadcn/ui `sidebar` — variante BASE (registre `base-nova`, primitives
 * @base-ui/react : `use-render` + `merge-props`), recopiée via le CLI
 * (`bunx shadcn add sidebar`, alias réécrits en `~/…` par components.json)
 * puis adaptée au projet :
 *  - `"use client"` retiré : TanStack Start, pas de RSC (aucun fichier du
 *    projet n'en porte) ;
 *  - `useIsMobile` vient de `~/lib/use-mobile` (pas de dossier hooks/), un
 *    hook RÉÉCRIT en `useSyncExternalStore` SSR-sûr — voir son en-tête ;
 *  - l'écriture `document.cookie` (persistance `sidebar_state` du registre)
 *    est conservée telle quelle, sous un biome-ignore
 *    (lint/suspicious/noDocumentCookie) — un cookie de confort UI, pas un
 *    secret ;
 ...
 *    contrairement à cmdk ; les variantes présence-seule (`data-active:`,
 *    `data-open:`) du registre sont donc correctes ici, rien à corriger.
 * Tous les `data-slot` du registre sont conservés (le CSS print s'y accroche).
 */
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: stock shadcn persistence, documented in header
...
function SidebarMenuSkeleton({
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* Ternaire (et non `&&`) : la convention du projet, imposée par la
          règle noLeakedRender de Biome. */}
      {showIcon ? (
        <Skeleton
