/**
 * shadcn/ui `sheet` — variante BASE (registre `base-nova`, primitives
 * @base-ui/react/dialog), recopiée via le CLI (alias réécrits en `~/…` par
 * components.json) puis adaptée au projet :
 *  - ternaire au lieu de `&&` pour le bouton de fermeture (règle
 *    noLeakedRender de Biome, même convention que dialog.tsx) ;
 *  - audit des variantes `data-*` : `data-[side=…]` teste une VALEUR (posée
 *    par le composant lui-même), `data-starting-style:`/`data-ending-style:`
 *    sont des attributs présence-seule de Base UI — pas de bug
 *    `data-…="false"` à la cmdk, rien à corriger.
 * Tous les `data-slot` du registre sont conservés. Ajouté comme dépendance
 * de `sidebar` (le tiroir mobile).
 */
