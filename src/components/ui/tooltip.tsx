/**
 * shadcn/ui `tooltip` — variante BASE (registre `base-nova`, primitives
 * @base-ui/react/tooltip), recopiée via le CLI (alias réécrits en `~/…` par
 * components.json) puis adaptée au projet :
 *  - `"use client"` retiré : TanStack Start, pas de RSC ;
 *  - audit des variantes `data-*` : `data-open:`/`data-closed:` sont des
 *    attributs présence-seule de Base UI (un seul des deux à la fois),
 *    `data-[side=…]` et `data-[state=delayed-open]` testent une VALEUR —
 *    pas de bug `data-…="false"` à la cmdk, rien à corriger.
 * Tous les `data-slot` du registre sont conservés. Ajouté comme dépendance
 * de `sidebar` (l'info-bulle des boutons en mode icône).
 */
